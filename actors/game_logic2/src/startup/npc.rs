    ...
    for n_x in 15..19{
      for n_y in 15..19{
        for v in 0..z%3{
          ...
          z = z+sign;
          let x1 = 200*n_x + x+sign*sign +v*sign;
          let y1 = 200*n_y + y+sign*sign + sign*v;
          npc_positions_vec.push((Position(Vec2::new(x1 as f32,y1 as f32)),sprite_enum));
          let sign = z %11;
          z = z+sign;
          let sprite_enum = if (z+v)%3 ==0 {0} else if (z+v)%3 ==1 {1 }else {2};
          let x2 = 200*n_x + x+sign*sign - v*sign;
          let y2 = 200*n_y + y-sign*sign - sign*v;
          npc_positions_vec.push((Position(Vec2::new(x2 as f32,y2 as f32)),sprite_enum));
          let sign3 = z %11;
          z = z + sign3;
          let sprite_enum = if (z+v)%3 ==0 {0} else if (z+v)%3 ==1 {1 }else {2}; 
          let x3 = 200*n_x + x - sign3*sign +v*sign;
          let y3 = 200*n_y + y + sign3*sign - v*sign;
